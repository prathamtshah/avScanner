...
RUN yum install -y python3.8 python3.8-pip cpio yum-utils zip unzip less 
...
ENV LD_LIBRARY_PATH=/opt/app/bin:/usr/lib64:/lib64:/usr/local/lib64:$LD_LIBRARY_PATH

# Download libraries we need to run in lambda
WORKDIR /tmp
RUN yumdownloader -x \*i686 --archlist=x86_64 clamav clamav-lib clamav-update json-c pcre libprelude gnutls libtasn1 lib64nettle nettle bzip2-libs libtool-ltdl libxml2 libcurl xz-libs nghttp2
...
RUN rpm2cpio nghttp2*.rpm | cpio -idmv
